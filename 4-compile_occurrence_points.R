...
### DESCRIPTION:
  ## This script compiles in situ occurrence point data that were downloaded in
  ...
  #   then records are saved in taxon-specific CSV files. There is also an 
  #   option to add an elevationInMeters column that can be used later for 
  #   filtering taxon points.
...
geo_pts$country <- mgsub(geo_pts$country,
    c("estados unidos","estados unidos de america","mexique","México","etats-unis d'amérique","U. S. A."),#matched unambiguously
    c("United States","United States","Mexico","Mexico","United States","United States")) #country name to use
...
geo_pts$countryCode <- mgsub(geo_pts$countryCode,
    c("CAN","FRA","GBR","ISR","MEX","USA"), #matched unambiguously
    c("CA","FR","GB","IL","MX","US")) #2-letter code to use
